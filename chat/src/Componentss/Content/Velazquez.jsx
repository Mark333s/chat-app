import React from "react";
import { sendMessage3 } from "../../redux/state";
import s from './Velazquez.module.css';
import Messages from "./VelazquezItem/velazquez";


const Velazquez = (props) => {
    

    let messagesElements = 
    props.messagesData3.map(message=> <Messages src={message.img} text={message.text} data={message.data} />)
    
   
    let newMessageEl = React.createRef();
          
    let sendMessage3 = () => {
       let text = newMessageEl.current.value;
        props.sendMessage3(text) 
        newMessageEl.current.value = '';
    }
    return (
        <>
            <div className={s.main}>
                <div className={s.user}>
                    <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRLjRDrylE_ZBdMzZ_cO8nO8sYFXFFYjOPlQ&usqp=CAU' alt='boy' ></img>
                    <h5>Velazquez</h5>
                </div>

                <div className={s.texts}>
               {messagesElements}
 </div>



                <div className={s.yourMessage}>
                    <input placeholder="Type your message" className={s.input} ref={newMessageEl}></input>
                    <button onClick={sendMessage3}><img src='data:image/png;base64,' className={s.plane}></img></button>
                </div>


            </div>
        </>
    )
}



export default Velazquez;